...
export default function ReturKonsinyasi() {
    const {
        ...
        nextNoRetur = "",
    } = usePage<any>().props as unknown as PageProps;

    const [isFormOpen, setIsFormOpen] = useState(false);
    const [viewingDetail, setViewingDetail] = useState<HistoryRetur | null>(
        null,
    );
    const [searchTerm, setSearchTerm] = useState("");
    const [editingId, setEditingId] = useState<number | null>(null);

    const kondisiBarangList: ("Layak" | "Perlu Perbaikan" | "Rusak")[] = [
        "Layak",
        "Perlu Perbaikan",
        "Rusak",
    ];
    ...
    const { data, setData, post, put, transform, processing, reset } =
    ...
    const handleItemRowChange = (
        ...
        if (field === "qty") {
            ...
            if (inputQty > item.qty_kirim) {
                alert(
                    `Jumlah retur tidak boleh melebihi qty terkirim (${item.qty_kirim})`,
                );
        ...
        updatedItems.forEach((itm) => {
            if (itm.isRetured && itm.qty > 0) {
                const masterProduk = dataProdukKonsinyasi.find(
                    (p) => p.id_produk === parseInt(itm.id_produk as string),
                );
                const hargaSatuan = masterProduk
                    ? parseInt(masterProduk.harga as any) || 0
                    : 0;
                const subtotalItem = hargaSatuan * itm.qty;

                if (itm.kondisi_barang === "Perlu Perbaikan")
                    kalkulasiTotalPerbaikan += subtotalItem;
                if (itm.kondisi_barang === "Rusak")
                    kalkulasiTotalKerugian += subtotalItem;
            }
        });
    ...
    const activeItems = data.items.filter(
        (item) => item.isRetured && item.qty > 0,
    );
    ...
    const handleSubmit = () => {
        ...
        const itemAkanDiretur = data.items.filter(
            (item) => item.isRetured && item.qty > 0,
        );

        if (itemAkanDiretur.length === 0) {
            alert(
                "Minimal harus ada 1 produk dengan jumlah retur valid (> 0)!",
            );
            return;
        }
        if (itemAkanDiretur.some((item) => !item.kondisi_barang)) {
            alert(
                "Mohon lengkapi Kondisi Barang untuk produk yang akan diretur.",
            );
            return;
        }
        ...
        transform((formData) => {
            ...
            const finalItems = formData.items
                ...
                .map((item) => {
                    const masterProduk = dataProdukKonsinyasi.find(
                        (p) =>
                            p.id_produk ===
                            (typeof item.id_produk === "string"
                                ? parseInt(item.id_produk)
                                : item.id_produk),
                    );
                    const hargaSatuan = masterProduk
                        ? parseInt(masterProduk.harga as any) || 0
                    ...
                    if (item.kondisi_barang === "Perlu Perbaikan")
                        kalkulasiTotalPerbaikan += subtotalItem;
            ...
            return {
                ...formData,
                id_konsinyasi_keluar: parseInt(
                    formData.id_konsinyasi_keluar as string,
                ),
                total_perbaikan: kalkulasiTotalPerbaikan,
                total_kerugian: kalkulasiTotalKerugian,
                items: finalItems,
            ...
            <div className="p-8">
                <div className="mb-6">
                    <button
                        ...
                        className="text-red-600 hover:underline mb-2 flex items-center gap-1 font-medium text-sm"
                    >
                        <ArrowLeft className="w-4 h-4" /> Kembali ke Daftar
                    </button>
                    <h1 className="text-3xl font-bold text-gray-900">
                        Detail Retur {viewingDetail.no_retur_konsinyasi}
                    </h1>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                        <div>
                            <span className="text-gray-500">Mitra / Toko:</span>{" "}
                            <p className="font-semibold text-gray-800">
                                {viewingDetail.nama_toko}
                            </p>
                        </div>
                        <div>
                            ...
                            <p className="font-semibold text-gray-800">
                                {viewingDetail.no_konsinyasi_keluar}
                            </p>
                        </div>
                        <div>
                            <span className="text-gray-500">
                            ...
                            <p className="font-semibold text-gray-800">
                                {viewingDetail.tgl_retur_konsinyasi}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={data.no_retur_konsinyasi}
                                    disabled
                                    className="w-full border border-gray-200 bg-gray-50 rounded-lg p-2 text-sm text-gray-500 outline-none"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                ...
                                <input
                                    type="date"
                                    value={data.tgl_retur_konsinyasi}
                                    onChange={(e) =>
                                        setData(
                                            "tgl_retur_konsinyasi",
                                            e.target.value,
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    Pilih No. Konsinyasi Asal
                                </label>
                                <select
                                    value={data.id_konsinyasi_keluar}
                                    onChange={(e) =>
                                        handleKonsinyasiChange(e.target.value)
                                    }
                                ...
                                >
                                    ...
                                    {dataKonsinyasiKeluar.map((kon) => (
                                        <option
                                            key={kon.id_konsinyasi_keluar}
                                            value={kon.id_konsinyasi_keluar}
                                        >
                                            {kon.no_dokumen}
                                        </option>
                                    ))}
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={
                                        dataKonsinyasiKeluar.find(
                                            (k) =>
                                                k.id_konsinyasi_keluar ===
                                                parseInt(
                                                    data.id_konsinyasi_keluar as string,
                                                ),
                                        )?.nama_toko || ""
                                    }
                                    disabled
                                    className="w-full border border-gray-200 rounded-lg p-2 text-sm bg-gray-50 text-gray-500 cursor-not-allowed outline-none"
                                    placeholder="Otomatis terisi berdasarkan dokumen"
                                />
                            </div>
                        </div>

                        {data.id_konsinyasi_keluar &&
                        data.items.some((item) => item.isRetured) ? (
                            <div className="space-y-3">
                                <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">
                                    Daftar Produk Dikirim (Sesuaikan Jumlah
                                    Retur)
                                </h3>
                            ...
                            <div className="text-center py-6 text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                Semua produk dokumen telah dieliminasi dari
                                daftar retur. Silakan pilih ulang nomor dokumen
                                untuk mereset.
                            </div>
                        ) : (
                            <div className="text-center py-8 text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                Silakan pilih Nomor Konsinyasi Asal terlebih
                                dahulu untuk memuat daftar produk.
                            </div>
                        )}
                        ...
                        <div className="pt-4 border-t border-gray-200 flex justify-end gap-3">
                            ...
                            <button
                                ...
                                className={`px-6 py-2 rounded-lg text-sm font-medium text-white transition-colors shadow-sm ${activeItems.length === 0 ? "bg-gray-300 cursor-not-allowed text-gray-500" : "bg-red-800 hover:bg-red-900"}`}
                            >
                                {editingId
                                    ? "Simpan Perubahan"
                                    : "Simpan Retur"}
                ...
                <div className="space-y-6">
                    <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-red-800/80 mt-1 text-sm">
                                Kelola data pengembalian barang dari mitra
                                konsinyasi
                            </p>
                        </div>
                        <button
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <div className="px-6 py-4">
                            <div className="relative w-full">
                                ...
                                <input
                                    type="text"
                                    placeholder="Cari No. Retur, Konsinyasi, atau Mitra..."
                                    value={searchTerm}
                                    onChange={(e) =>
