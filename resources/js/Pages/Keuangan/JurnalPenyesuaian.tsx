...
interface JurnalPenyesuaianProps {
  jurnals?: any[];
}

export default function JurnalPenyesuaian({ jurnals = [] }: JurnalPenyesuaianProps) {
  ...
  // Mengubah struktur data dari Laravel agar sesuai dengan format UI
  ...
  // Perbaikan tipe data untuk TypeScript
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text('Jurnal Penyesuaian', pageWidth / 2, 21, { align: 'center' });
    ...
    // 3. STYLE TABEL (Seperti Purchase Order)
    autoTable(doc, {
      head: [['No. Jurnal', 'Tanggal', 'Ref', 'Nama Akun', 'Keterangan', 'Debit (Rp)', 'Kredit (Rp)']],
      body,
      startY: 40,
      theme: 'grid', // Mengubah tabel menjadi bergaris kotak-kotak hitam putih
      styles: { 
        fontSize: 9, 
        cellPadding: 3,
      ...
      didParseCell: function (data) {
        // Membuat baris "TOTAL" paling bawah menjadi Bold
        if (data.row.index === body.length - 1) {
          data.cell.styles.fontStyle = 'bold';
          data.cell.styles.fillColor = [250, 250, 250]; // Sedikit abu-abu untuk pembeda
          if (data.column.index === 4) {
             data.cell.styles.halign = 'right'; // Tulisan TOTAL rata kanan
          }
        }
      }
    });
    ...
    // 4. SIMPAN PDF
    doc.save(`JurnalPenyesuaian-${tanggalMulai}-${tanggalSelesai}.pdf`);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Jurnal Penyesuaian</h2>
          <p className="text-sm text-gray-500 mt-1">Jurnal dibuat otomatis dari proses penyesuaian akhir periode</p>
        </div>
