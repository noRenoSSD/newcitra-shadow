...
export default function SuratJalanForm({ pesanan, konsinyasi, onBack }: Props) {
    ...
    const idReferensi = isKonsinyasi
        ? konsinyasi?.id_konsinyasi_keluar
        : pesanan?.id_pesanan;
    const noDokumen = isKonsinyasi
        ? konsinyasi?.no_konsinyasi
        : pesanan?.no_pesanan;
    const namaTujuan = isKonsinyasi
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isKonsinyasi) {
            ...
            if (!finalId || finalId === "undefined") {
                alert(
                    "⚠️ Gagal: ID Konsinyasi tidak terbaca (undefined). Periksa kembali data!",
                );
                return;
            }

            router.post(
                `/konsinyasi-keluar/generate-sj/${finalId}`,
                {
        ...
        <div className="p-6 max-w-4xl mx-auto bg-white my-6 shadow-sm border border-gray-200 rounded-xl">
            ...
            <div className="flex items-center gap-4 mb-8 border-b border-gray-100 pb-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">
                        Dokumen instruksi pengiriman barang ke mitra.
                    </p>
                </div>
            </div>
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
                <p className="text-sm text-gray-600">
                    Referensi {isKonsinyasi ? "Konsinyasi" : "SO"}:{" "}
                    <span className="font-bold text-gray-900">{noDokumen}</span>
