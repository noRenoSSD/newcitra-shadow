...
import { Plus, Search, Eye, Pencil, Trash2, FileText, Truck } from 'lucide-react';
        ...
        <div className="block">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div><p className="text-xs font-semibold text-gray-400">No. Pesanan</p><p className="text-base font-bold text-gray-900">{selectedOrder.no_pesanan}</p></div>
              <div><p className="text-xs font-semibold text-gray-400">Tanggal</p><p className="text-base font-medium text-gray-900">{new Date(selectedOrder.tgl_pesanan).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })}</p></div>
              <div><p className="text-xs font-semibold text-gray-400">Jenis Transaksi</p><p className="text-base font-bold text-gray-900">{selectedOrder.jenis_transaksi}</p></div>
              <div><p className="text-xs font-semibold text-gray-400">Pelanggan</p><p className="text-base font-bold text-gray-900">{selectedOrder.nama_mitra || 'Tidak Diketahui'}</p></div>
              <div className="md:col-span-2"><p className="text-xs font-semibold text-gray-400">Alamat Pengiriman</p><p className="text-base font-medium text-gray-900">{selectedOrder.alamat || '-'}</p></div>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            ...
            <div className="bg-white rounded-xl border p-6 w-full md:w-80 space-y-1.5 text-sm text-gray-500">
              <div className="flex justify-between"><span>Total Potongan Diskon:</span><span className="text-red-600 font-medium">Rp {(Number(selectedOrder.total_diskon) || 0).toLocaleString('id-ID')}</span></div>
              <div className="flex justify-between font-bold text-gray-900 border-t pt-2 text-base"><span>Total Tagihan:</span><span className="text-red-800">Rp {(Number(selectedOrder.total_harga) || 0).toLocaleString('id-ID')}</span></div>
            </div>
          </div>
        </div>
      )}
                ...
                <tbody className="divide-y text-sm">
                  {filteredOrders.length === 0 ? (<tr><td colSpan={7} className="text-center py-8 text-gray-500">Tidak ada data dokumen pesanan</td></tr>) : (
                    filteredOrders.map((item) => {
                      const isLocked = item.sudah_ada_invoice || item.status_surat_jalan;
                      return (
                        <tr key={item.id_pesanan} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${item.sudah_ada_invoice ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${item.sudah_ada_invoice ? 'bg-emerald-500' : 'bg-amber-500'}`} />{item.sudah_ada_invoice ? 'Selesai' : 'Diproses'}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-center">
                            <div className="flex items-center justify-center gap-2">
                              <button onClick={() => handleDetail(item)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-md"><Eye className="w-4 h-4" /></button>
                              <button disabled={isLocked} onClick={() => handleEdit(item)} className={`p-1.5 rounded-md ${isLocked ? 'text-gray-300 bg-gray-50 cursor-not-allowed' : 'text-yellow-600 hover:bg-yellow-50'}`}><Pencil className="w-4 h-4" /></button>
                              <button disabled={isLocked} onClick={() => handleDelete(item.id_pesanan)} className={`p-1.5 rounded-md ${isLocked ? 'text-gray-300 bg-gray-50 cursor-not-allowed' : 'text-red-600 hover:bg-red-50'}`}><Trash2 className="w-4 h-4" /></button>
                              <button type="button" onClick={() => router.get(`/invoice/create?so_id=${item.id_pesanan}`)} className="p-1.5 text-gray-600 hover:bg-gray-50 rounded-md"><FileText className="w-4 h-4" /></button>
                              <button type="button" disabled={item.status_surat_jalan} onClick={() => !item.status_surat_jalan && router.get(`/delivery-order/create?so_id=${item.id_pesanan}`)} className={`p-1.5 rounded-md ${item.status_surat_jalan ? 'text-gray-300 bg-gray-50 cursor-not-allowed' : 'text-purple-600 hover:bg-purple-50'}`}><Truck className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
                      );
