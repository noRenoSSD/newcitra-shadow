...
export default function ReturPenjualan({
    ...
    const [returPenjualans, setReturPenjualans] = useState<
        ReturPenjualanItem[]
    >(formatReturData(listRetur));
    const [invoicesFromAPI, setInvoicesFromAPI] =
        useState<InvoiceDataAPI[]>(listInvoice);
    ...
    const [viewingDetail, setViewingDetail] =
        useState<ReturPenjualanItem | null>(null);
    ...
    const handleInvoiceChange = (noInv: string) => {
        const selectedInvoice = listInvoice.find(
            (inv) => inv.no_jual === noInv,
        );
        ...
        if (selectedInvoice) {
            ...
            const mappedItems = apiItems.map((item: any) => {
                const qtyBeli =
                    Number(item.qty_terjual) || Number(item.qty_jual) || 0;
                ...
                else if (Number(item.harga_jual_satuan) > 0)
                    hargaBeli = Number(item.harga_jual_satuan);
                else if (Number(item.harga_satuan) > 0)
                    hargaBeli = Number(item.harga_satuan);
                else {
    ...
    const handleItemRowChange = (index: number, field: string, value: any) => {
        ...
        if (field === "qty") {
            ...
            if (inputQty > item.qtyTerjual) {
                alert(
                    `Jumlah retur tidak boleh melebihi qty terjual (${item.qtyTerjual})`,
                );
    ...
    const calculateSubtotal = () =>
        activeItems.reduce((sum, item) => sum + item.subtotal, 0);
    ...
    const handleSubmit = () => {
        ...
        const missingCondition = activeItems.some(
            (item) => !item.kondisiBarang,
        );
        if (missingCondition) {
            alert(
        ...
        // PERBAIKAN: Menyisipkan no_retur_jual dan tgl_retur_jual ke dalam payload
        const payload = {
            ...
            total_perbaikan: activeItems
                .filter((i) => i.kondisiBarang === "Perlu Perbaikan")
                .reduce((s, i) => s + i.subtotal, 0),
            total_kerugian: activeItems
                .filter((i) => i.kondisiBarang === "Rusak")
                .reduce((s, i) => s + i.subtotal, 0),
            items: activeItems.map((item) => ({
                ...
                biaya_perbaikan: 0,
                nilai_kerugian:
                    item.kondisiBarang === "Rusak" ? item.subtotal : 0,
                keterangan: item.keterangan,
            })),
        };
            ...
            {isFormOpen ? (
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
                        ...
                        {formData.noInvoice &&
                        items.some((item) => item.isRetured) ? (
                            <div className="space-y-3">
                                <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">
                                    Daftar Produk Terjual (Sesuaikan Jumlah
                                    Retur)
                                </h3>
                            ...
                            <div className="text-center py-6 text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                Semua produk invoice telah dieliminasi dari
                                daftar retur.
                            </div>
                        ) : (
                            <div className="text-center py-8 text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                Silakan pilih Nomor Invoice Asal terlebih
                                dahulu.
                            </div>
                        )}
                    ...
                    <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-red-800/80 mt-1 text-sm">
                                Kelola data pengembalian barang dari pelanggan
                            </p>
                        </div>
                        <button
                            onClick={handleOpenNewForm}
                            className="bg-red-800 hover:bg-red-900 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-medium transition-colors shadow-sm"
                        >
